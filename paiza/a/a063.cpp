...
int main()
{
  ...
  auto it = lower_bound(m.begin(), m.end(), (sum + 1) / 2);
  int mid = it - m.begin();
  REP(i, mid)
  {
    a.push_back({m[i], x[i]});
  ...
  if (mid != (sum + 1) / 2)
  {
    b.push_back({0, x[mid - 1]});
  }
  REP2(i, mid, n)
  {
    b.push_back({m[i] - (sum + 1) / 2, x[i]});
  }

  int aind = 0, bind = 0;
  int ch = 0;
  ll ans = 0;
  while (aind + 1 != a.size() && bind + 1 != b.size())
  {
    if (a[aind + 1].first < b[bind + 1].first)
    {
      ans += (a[aind + 1].first - ch) * abs(a[aind].second - b[bind].second);
      ch = a[aind + 1].first;
      aind++;
    }
    else if (a[aind + 1].first > b[bind + 1].first)
    {
      ans += (b[bind + 1].first - ch) * abs(a[aind].second - b[bind].second);
      ch = b[bind + 1].first;
      bind++;
    }
    else
  ...
  ans += ((sum + 1) / 2 - ch) * abs(a[aind].second - b[bind].second);
