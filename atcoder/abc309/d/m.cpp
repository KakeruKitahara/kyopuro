...
int main()
{
  ...
  while (que.size() != 0)
  {
    ...
    REP(i, ed[p.first].size())
    {
      if (used[ed[p.first][i]] == 0)
      {
        used[ed[p.first][i]] = 1;
        chmax(ans1, p.second + 1);
        que.push({ed[p.first][i], p.second + 1});
      }
    }
  }
  ...
  while (que2.size() != 0)
  {
    ...
    REP(i, ed[p.first].size())
    {
      ...
      {
        used[ed[p.first][i]] = 1;
        chmax(ans2, p.second + 1);
        que2.push({ed[p.first][i], p.second + 1});
      }
    }
  }
