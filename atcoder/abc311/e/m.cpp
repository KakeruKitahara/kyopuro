...
int main()
{
  ...
  Vl x2(w, h - 1), y2(h, w - 1);
  lazy_segtree<ll, op, e, ll, mapping, composition, id> x(x2), y(y2);

  REP(i, n)
  {
    if (y.get(ab[i].first) > ab[i].second)
    {
      y.set(ab[i].first, ab[i].second);
    }

    if (x.get(ab[i].second) > ab[i].first)
    {
      x.set(ab[i].second, ab[i].first);
    }
  }
  ...
  REP(i, h)
  {
    REP(j, w)
    {
      int rowmin = y.prod(i, h);
      int colmin = x.prod(j, rowmin);

      ans += max(min(rowmin, colmin) + 1, 0);
    }
  }
