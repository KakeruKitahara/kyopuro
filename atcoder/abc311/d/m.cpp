...
int main()
{
  ...
  while (que.size() != 0)
  {
    pair<P, int> t = que.front();
    int now = t.second;
    P p = t.first;
    que.pop();

    P w[4] = {{-1, 0}, {1, 0}, {0, 1}, {0, -1}};
    ...
    if (now == 5)
    {
      ...
      {
        P ne = p + w[i];
        if (s[ne.first][ne.second] != '#' && mp[ne.first][ne.second][i] == 0)
        {
          mp[ne.first][ne.second][i] = 1;
          b[ne.first][ne.second] = '@';
          que.push({ne, i});
        }
      }
    }
    ...
    {
      P ne = p + w[now];
      if (s[ne.first][ne.second] != '#' && mp[ne.first][ne.second][now] == 0)
      {
        mp[ne.first][ne.second][now] = 1;
        b[ne.first][ne.second] = '@';
        que.push({ne, now});
      }
      else
