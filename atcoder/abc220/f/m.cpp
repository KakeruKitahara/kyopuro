...
pair<ll, int> dfs(int p, int f)
{
  ...
  REP(i, edge[p].size())
  {
    if (used[edge[p][i]] == 0)
    {
      pair<ll, int> tmp = dfs(edge[p][i], f + 1);
      sum.first += tmp.first;
...
void dfs2(int p, int mi, int su, ll pre)
{
  used[p] = 1;
  ans[p] = pre - mi + su;
  REP(i, edge[p].size())
  {
    if (used[edge[p][i]] == 0)
    {
      dfs2(edge[p][i], node[edge[p][i]], n - node[edge[p][i]], ans[p]);
    }
  }
}
