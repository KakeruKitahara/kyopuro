...
int main()
{
  ...
  REP2(i, 1, s2.size() + 1)
  {
    if (s2[i - 1].first == 0)
    {
      if (i == 1)
      {
        dp[i][0] = dp[i - 1][0] + x * s2[i - 1].second;
        dp[i][1] = IINF;
      }
      else
      {
        dp[i][0] = min(dp[i - 1][0] + x * s2[i - 1].second, dp[i - 1][1] + z + x * s2[i - 1].second);
        dp[i][1] = dp[i - 1][1] + y * s2[i - 1].second;
      }
    }
    else
    {
      if (i == 1)
      {
        dp[i][1] = dp[i - 1][0] + z + x * s2[i - 1].second;
        dp[i][0] = dp[i - 1][0] + y * s2[i - 1].second;
      }
      else
      {
        dp[i][1] = min(dp[i - 1][1] + x * s2[i - 1].second, dp[i - 1][0] + z + x * s2[i - 1].second);
        dp[i][0] = dp[i - 1][0] + y * s2[i - 1].second;
      }
    }
  }

  cout << min(dp[s2.size()][0], dp[s2.size()][1]) << endl;
  return 0;
}