...
int main()
{
  ...
  vector<Pl> sum;
  sum.push_back(make_pair(0, -s));
  REP(i, n)
  {
    ll t = s * (a[i + 1] - a[i]);
    sum.push_back(make_pair(t + sum.back().first, -s));
    s--;
  }
  Pl pa = make_pair(k, -IINF);
  sort(sum.begin(), sum.end());
  auto p = lower_bound(sum.begin(), sum.end(), pa);
  p--;

  REP(i, n)
  {
    int idx = p - sum.begin();
    b[i] = max(0ll, b[i] - a[idx]);
  }
